...
:   ![][cli-w] ![][env-w] ![][config-r] ![][bitbake-r]
    ...
    The default value is `OB_ROOT_DIR/build`.

    The command line option `O=dir` can be used to set the value.

<a id="OB_DEFCONFIG_DIR"></a>
OB_DEFCONFIG_DIR
:   ![][wizard-w1] ![][config-r]

    The directory where all the `defconfig` files can be found.

<a id="OB_PROJECT_ID"></a>
OB_PROJECT_ID
:   ![][wizard-w1] ![][config-r]

    A unique identifier for the project. Used to name the container image so
    that multiple OpenBar projects on the same host do not share images.
...
:   ![][wizard-w1] ![][config-r]
    ...
    The project type. One of `simple`, `initenv`, or `yocto`.

    See [Project Types](../user-guide/project-types.md) for details.
...
[config-file]: ../user-guide/configuration.md
[shell-target]: ../user-guide/usage.md#the-shell-target

<a id="OB_AUTO_TARGETS"></a>
OB_AUTO_TARGETS
:   ![][config-r]

    Extends to a list of all targets that are marked as automatic — i.e. those
    that will run when `make` is called with no arguments. Deduced from the
    lowercase naming convention first, then filtered by
    [`OB_MANUAL_TARGETS`](#OB_MANUAL_TARGETS).

<a id="OB_EXPORT"></a>
OB_EXPORT
:   ![][env-w] ![][config-rw]

    Space-separated list of variable names whose values are exported from the
    host environment into the container and into the `.config` file.

    The `override VAR = value` pattern in a defconfig file automatically adds
    `VAR` to `OB_EXPORT`.

    For more information, see [Exporting environment variables][export-variables].

[export-variables]: ../user-guide/configuration.md#exporting-environment-variables

<a id="OB_MANUAL_TARGETS"></a>
OB_MANUAL_TARGETS
:   ![][config-rw]

    Space-separated list of targets that are marked as manual. Manual targets
    do not run automatically when `make` is called with no arguments.

    By default, uppercase and symbol targets are manual. This variable overrides
    the naming convention for specific targets.
...
<a id="OB_CONTAINER_FILE"></a>
OB_CONTAINER_FILE
:   ![][env-w] ![][config-w]

    The full path to the container file.

    The default value is `OB_CONTAINER_CONTEXT/OB_CONTAINER_FILENAME`.

<a id="OB_CONTAINER_FILENAME"></a>
OB_CONTAINER_FILENAME
:   ![][env-w] ![][config-w]
...
<a id="OB_CONTAINER_FORCE_BUILD"></a>
OB_CONTAINER_FORCE_BUILD
:   ![][cli-w] ![][env-w] ![][config-w]

    Force a rebuild of the container image even if it already exists.

    Can also be set from the command line with `B=1`.

<a id="OB_CONTAINER_FORCE_PULL"></a>
OB_CONTAINER_FORCE_PULL
:   ![][cli-w] ![][env-w] ![][config-w]

    Force a pull of the container image from the registry.

    Can also be set from the command line with `P=1`.

<a id="OB_CONTAINER_HOME"></a>
OB_CONTAINER_HOME
:   ![][config-r]
...
<a id="OB_CONTAINER_IMAGE"></a>
OB_CONTAINER_IMAGE
:   ![][env-w] ![][config-w]

    A pre-built container image to pull instead of building from a Dockerfile.

    When set, OpenBar pulls this image rather than building locally.

<a id="OB_CONTAINER_POLICY"></a>
OB_CONTAINER_POLICY
:   ![][env-w] ![][config-w]

    The policy for obtaining the container image. One of:

    - `always` — always build or pull.
    - `newer` — pull or build only if a newer version is available.
    - `missing` — only if the image is not already present locally (default).
    - `never` — never build or pull; fail if the image is missing.

<a id="OB_CONTAINER_RUN_EXTRA_ARGS"></a>
OB_CONTAINER_RUN_EXTRA_ARGS
:   ![][env-w] ![][config-w]

    Additional argument to be given during the container `run` command,
    for both `podman` and `docker` engine.

<a id="OB_CONTAINER_VOLUMES"></a>
OB_CONTAINER_VOLUMES
:   ![][env-w] ![][config-w]
    ...
    By default `OB_ROOT_DIR` and `OB_BUILD_DIR` are already mounted.
...
<a id="OB_DOCKER_GROUPS"></a>
OB_DOCKER_GROUPS
:   ![][env-w] ![][config-w]

    Additional groups to assign to the user inside a Docker container.

    Not needed for Podman, which handles user namespaces differently.

<a id="OB_PODMAN_BUILD_EXTRA_ARGS"></a>
OB_PODMAN_BUILD_EXTRA_ARGS
:   ![][env-w] ![][config-w]

    Additional argument to be given during the `podman build` command.

<a id="OB_PODMAN_RUN_EXTRA_ARGS"></a>
OB_PODMAN_RUN_EXTRA_ARGS
:   ![][env-w] ![][config-w]
...
<a id="OB_DOCKER_BUILD_EXTRA_ARGS"></a>
OB_DOCKER_BUILD_EXTRA_ARGS
:   ![][env-w] ![][config-w]

    Additional argument to be given during the `docker build` command.

<a id="OB_DOCKER_RUN_EXTRA_ARGS"></a>
OB_DOCKER_RUN_EXTRA_ARGS
:   ![][env-w] ![][config-w]
...
<a id="OB_INITENV_SCRIPT"></a>
OB_INITENV_SCRIPT
:   ![][wizard-w1] ![][config-r]

    The path to the environment setup script sourced before running targets.

    Used by both the `initenv` and `yocto` project types. The default for
    Yocto projects is `oe-init-build-env`.

<a id="OB_YOCTO_EXPORT_VARIABLE"></a>
OB_YOCTO_EXPORT_VARIABLE
:   ![][env-w] ![][config-w]

    The name of the Bitbake variable used to pass external environment
    variables into Bitbake's scope.
    ...
    !!! note
        ...
        ``` make title=".config"
        OB_YOCTO_EXPORT_VARIABLE := BB_ENV_EXTRAWHITE
        ```
...
:   ![][env-w] ![][config-w]
    ...
    The path to the directory containing `local.conf.sample` and
    `bblayers.conf.sample` files used to initialise a new Yocto build
    directory.

    See [TEMPLATECONF][yocto-TEMPLATECONF] in the Yocto documentation.

[yocto-TEMPLATECONF]: https://docs.yoctoproject.org/ref-manual/variables.html#term-TEMPLATECONF
